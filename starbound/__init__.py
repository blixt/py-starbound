...
class WorldInfo(object):
    ...
    def _celestialParameters(self):
        ...
        r = t(None, None, None, set())
        cp = self.metadata.get('worldTemplate', {}).get('celestialParameters')
        if not cp:
            return r
        r.name = cp.get('name')
        if 'parameters' in cp:
            r.description = cp['parameters'].get('description')
            if 'terrestrialType' in cp['parameters']:
                r.biomes.update(cp['parameters']['terrestrialType'])
        if 'coordinate' in cp and 'location' in cp['coordinate']:
            r.coords = tuple(cp['coordinate']['location'])
        return r
